...
module tb_decimation_filter_top;
    ...
    wire audio_valid;
    wire signed [22:0] cic_out;
    wire cic_valid;
    wire signed [23:0] fir1_out;
    wire fir1_valid;
    ...
    initial begin
        ...
        repeat(10240) begin
            ...
            if (cic_valid) begin
                ...
                if (cic_count <= 10)
                    $display("[CIC Out %0d] Value = %0d", cic_count, cic_out);
            end
            
            if (fir1_valid) begin
                fir1_count = fir1_count + 1;
                if (fir1_count <= 10)
                    $display("[FIR1 Out %0d] Value = %0d", fir1_count, fir1_out);
            end
    ...
    // Waveform dump
    initial begin
        $dumpfile("decimation_filter.vcd");
        $dumpvars(0, tb_decimation_filter_top);
    end
